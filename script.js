...
const showDetailTag = document.getElementsByClassName("showDetail")[0];
...
autoCompleteTag.addEventListener("keyup", (e) => {
  ...
  const hasProductToShow = filterProduct.length > 0;
  if (hasProductToShow) {
    for (let i = 0; i < filterProduct.length; i++) {
      const productItemContainer = document.createElement("div");
      productItemContainer.id = filterProduct[i].id;
...
let indexToSelect = -1;
const navigateSelectProduct = (key) => {
  if (key === "ArrowDown") {
    ...
    // filterProduct = [product1, product2, product3, product4];
    indexToSelect -= 1;
    deselectProduct();
    const productItemContainerToSelect = selectProduct(indexToSelect);
    productItemContainerToSelect.classList.add("selected");
    ...
    showDetailTag.style.display = "block";

    const showDetailContainer = document.createElement("div");
    showDetailContainer.id = filterProduct[indexToSelect].id;
    showDetailContainer.classList.add("showDetailContainer");

    const showDetailName = document.createElement("div");
    showDetailName.classList.add("showDetailName");
    showDetailName.append(filterProduct[indexToSelect].title);

    const showDetailImage = document.createElement("img");
    showDetailImage.classList.add("showDetailImage");
    showDetailImage.src = filterProduct[indexToSelect].image;

    showDetailContainer.append(showDetailName, showDetailImage);
    showDetailTag.append(showDetailContainer);
  }
};
...
const selectProduct = (index) => {
  const productToSelect = filterProduct[index].id.toString();
